from flask import Flask, render_template, request, redirect, url_for
import mysql.connector
from mysql.connector import Error
from datetime import datetime

app = Flask(__name__)

# MySQL connection settings
db_config = {
    'host': 'localhost',
    'user': 'root',
    'password': '@Ftsvolvl9',
    'database': 'Worker_database'
}

# Function to connect to the database
def create_database_connection():
    try:
        connection = mysql.connector.connect(**db_config)
        if connection.is_connected():
            print("Connected to 'Worker_database'")
        return connection
    except Error as e:
        print("Error while connecting to 'Worker_database':", e)
        return None
    
# Initialize the database
def init_db():
    conn = create_database_connection()
    if conn:
        try:
            c = conn.cursor()
            
            # Create employees table if it does not exist
            c.execute('''CREATE TABLE IF NOT EXISTS employees (
                        id INT AUTO_INCREMENT PRIMARY KEY,
                        worker_id VARCHAR(255) UNIQUE,  
                        name VARCHAR(255) NOT NULL,
                        surname VARCHAR(255) NOT NULL,
                        dob DATE NOT NULL,
                        email VARCHAR(255) NOT NULL UNIQUE,
                        cellphone VARCHAR(255) NOT NULL UNIQUE, 
                        id_number VARCHAR(255) NOT NULL UNIQUE,
                        course VARCHAR(255) NOT NULL,
                        specialization VARCHAR(255) NOT NULL,
                        address TEXT NOT NULL,
                        cohort VARCHAR(255) NOT NULL,
                        UNIQUE(name, surname)
                    )''')
            print("Employees table created or already exists.")
            conn.commit()
        except mysql.connector.Error as err:
            print(f"Error creating employees table: {err}")
        finally:
            conn.close()

# Create a table for a specific cohort
def create_cohort_table(cohort_name):
    conn = create_database_connection()
    if conn:
        c = conn.cursor()

        # Format the cohort table name correctly (replace spaces and make lowercase)
        cohort_name_slug = cohort_name.replace(" ", "_").lower()

        # Create cohort-specific table
        create_table_sql = f'''
        CREATE TABLE IF NOT EXISTS {cohort_name_slug} (
                    id INT AUTO_INCREMENT PRIMARY KEY,
                    worker_id VARCHAR(255) UNIQUE,  
                    name VARCHAR(255) NOT NULL,
                    surname VARCHAR(255) NOT NULL,
                    dob DATE NOT NULL,
                    email VARCHAR(255) NOT NULL UNIQUE,
                    cellphone VARCHAR(255) NOT NULL UNIQUE,
                    id_number VARCHAR(255) NOT NULL UNIQUE,
                    course VARCHAR(255) NOT NULL,
                    specialization VARCHAR(255) NOT NULL,
                    address TEXT NOT NULL,
                    cohort VARCHAR(255) NOT NULL,
                    UNIQUE(name, surname)
        );
        '''
        c.execute(create_table_sql)
        conn.commit()
        conn.close()
        print(f"Table for {cohort_name} created or already exists.")

def get_employee_by_id(worker_id):
    conn = create_database_connection()
    if conn:
        c = conn.cursor()
        c.execute("SELECT name, surname FROM employees WHERE worker_id = %s", (worker_id,))
        result = c.fetchone()
        conn.close()
        if result:
            return {"name": result[0], "surname": result[1]}
    return None

@app.route("/")
def Home():
    return render_template("layout.html")


# this function checks if the same address has been used multiple times by different users 
def check_address(address):
    conn = create_database_connection()
    if conn:
        c = conn.cursor()

        try:
            # Query to count how many times the address has been used
            query = "SELECT COUNT(*) FROM employees WHERE address = %s"
            c.execute(query, (address,))
            result = c.fetchone()

            if result and result[0] >= 3:  # If address appears 3 or more times
                return "Same address has been used by other participants 3 or more times."
        except Error as e:
            print("Error while checking address in 'Worker_database':", e)
        finally:
            conn.close()

    return None  # Return None if the address is valid

@app.route("/Register", methods=['POST', 'GET'])
def Register():
    if request.method == 'POST':
        name = request.form['name']
        surname = request.form['surname']
        dob = request.form['dob']
        email = request.form['email']
        cellphone = request.form['cellphone']
        id_number = request.form['id_number']
        course = request.form['course']
        specialization = request.form['specialization']
        address = request.form['address']
        cohort = request.form['cohort']

        # Validate cellphone number (must be 10 digits)
        if not cellphone.isdigit() or len(cellphone) != 10:
            error = "Cellphone number must be exactly 10 digits."
            return render_template("employee.html", error=error)
        
        # Check if the address has been used 3 or more times
        address_error = check_address(address)
        if address_error:
            return render_template("employee.html", error=address_error)
        

        # Check if the cellphone already exists in the database
        conn = create_database_connection()
        if conn:
            c = conn.cursor()
            c.execute("SELECT COUNT(*) FROM employees WHERE cellphone = %s", (cellphone,))
            result = c.fetchone()
            if result[0] > 0:
                error = "This cellphone number is already registered."
                conn.close()
                return render_template("employee.html", error=error)

        # Check if the ID number already exists in the database
        if conn:
            c = conn.cursor()
            c.execute("SELECT COUNT(*) FROM employees WHERE id_number = %s", (id_number,))
            result = c.fetchone()
            if result[0] > 0:
                error = "This ID number is already registered."
                conn.close()
                return render_template("employee.html", error=error)
            
        # Validate ID number (must be 13 digits)
        if not id_number.isdigit() or len(id_number) != 13:
            error = "ID number must be exactly 13 digits."
            return render_template("employee.html", error=error)

        # Validate the date of birth
        try:
            birth_date = datetime.strptime(dob, '%Y-%m-%d')
        except ValueError:
            error = "Invalid date format. Use YYYY-MM-DD."
            return render_template("employee.html", error=error)

        today = datetime.now()
        age = (today - birth_date).days // 365  # Calculate age in years
        if age < 18 or age > 35:
            error = "Date of birth must be for individuals between 18 and 35 years old."
            return render_template("employee.html", error=error)

        conn = create_database_connection()
        if conn:
            c = conn.cursor()

            try:
                # Insert into employees table
                c.execute('''INSERT INTO employees (name, surname, dob, email, cellphone, id_number, course, specialization, address, cohort)
                             VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)''',
                          (name, surname, dob, email, cellphone, id_number, course, specialization, address, cohort))
                conn.commit()

                # Retrieve the worker ID and update it as before
                worker_id = c.lastrowid
                formatted_worker_id = f"{worker_id:03}"
                c.execute('UPDATE employees SET worker_id = %s WHERE id = %s', (formatted_worker_id, worker_id))
                conn.commit()

                # Insert into the correct cohort-specific course table
                cohort_table_name = f"{course.replace(' ', '_').lower()}_{cohort.replace(' ', '_').lower()}"
                c.execute(f"SELECT table_name FROM information_schema.tables WHERE table_name = %s", (cohort_table_name,))
                if not c.fetchone():
                    # Create cohort-specific table if it doesn't exist
                    c.execute(f'''
                        CREATE TABLE {cohort_table_name} (
                            id INT AUTO_INCREMENT PRIMARY KEY,
                            worker_id VARCHAR(255) UNIQUE,
                            name VARCHAR(255) NOT NULL,
                            surname VARCHAR(255) NOT NULL,
                            dob DATE NOT NULL,
                            email VARCHAR(255) NOT NULL UNIQUE,
                            cellphone VARCHAR(255) NOT NULL,
                            id_number VARCHAR(255) NOT NULL,
                            course VARCHAR(255) NOT NULL,
                            specialization VARCHAR(255) NOT NULL,
                            address TEXT NOT NULL,
                            cohort VARCHAR(255) NOT NULL,
                            UNIQUE(name, surname)
                        );
                    ''')
                    conn.commit()

                c.execute(f'''INSERT INTO {cohort_table_name} 
                             (worker_id, name, surname, dob, email, cellphone, id_number, course, specialization, address, cohort) 
                             VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)''',
                          (formatted_worker_id, name, surname, dob, email, cellphone, id_number, course, specialization, address, cohort))
                conn.commit()

                # Insert into specialization-specific table
                specialization_table = specialization.replace(" ", "_").lower()
                c.execute(f"SELECT table_name FROM information_schema.tables WHERE table_name = %s", (specialization_table,))
                if not c.fetchone():
                    # Create specialization-specific table if it doesn't exist
                    c.execute(f'''
                        CREATE TABLE {specialization_table} (
                            id INT AUTO_INCREMENT PRIMARY KEY,
                            worker_id VARCHAR(255) UNIQUE,
                            name VARCHAR(255) NOT NULL,
                            surname VARCHAR(255) NOT NULL,
                            dob DATE NOT NULL,
                            email VARCHAR(255) NOT NULL UNIQUE,
                            cellphone VARCHAR(255) NOT NULL,
                            id_number VARCHAR(255) NOT NULL,
                            course VARCHAR(255) NOT NULL,
                            specialization VARCHAR(255) NOT NULL,
                            address TEXT NOT NULL,
                            cohort VARCHAR(255) NOT NULL,
                            UNIQUE(name, surname)
                        );
                    ''')
                    conn.commit()

                c.execute(f'''INSERT INTO {specialization_table} 
                             (worker_id, name, surname, dob, email, cellphone, id_number, course, specialization, address, cohort) 
                             VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)''',
                          (formatted_worker_id, name, surname, dob, email, cellphone, id_number, course, specialization, address, cohort))
                conn.commit()

            except mysql.connector.IntegrityError as e:
                conn.close()
                if "email" in str(e):
                    error = "The email address is already registered. Please use a different email."
                elif "name, surname" in str(e):
                    error = "A person with the same name and surname is already registered."
                else:
                    error = "An error occurred during registration. Please try again."
                return render_template("employee.html", error=error)

            conn.close()
            return redirect(url_for('Register_OUT', worker_id=formatted_worker_id))

    return render_template("employee.html")


@app.route("/Register_OUT")
def Register_OUT():
    worker_id = request.args.get('worker_id', None)
    if worker_id:
        worker_id = f"{int(worker_id):03}"  # Format worker_id as a zero-padded 3-digit string

        # Retrieve employee details
        employee = get_employee_by_id(worker_id)
        if employee:
            name = employee.get("name")
            surname = employee.get("surname")
        else:
            name, surname = None, None
    else:
        name, surname = None, None

    return render_template("Register_OUT.html", worker_id=worker_id, name=name, surname=surname)

# Call init_db to create tables on app start
if __name__ == "__main__":
    init_db()
    app.run(debug=True)
